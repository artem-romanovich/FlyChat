...
public class InternetActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            FirebaseDatabase database = FirebaseDatabase.getInstance();
            final DatabaseReference last_time_userRef = database.getReference(secret_field + "/Online_Users/" + interlocutor_nic + "/" + interlocutor_nic);
            ...
            NewMessageAdapterMoreLays adapterMoreLays = new NewMessageAdapterMoreLays(InternetActivity.this, MESSAGES, messages, my_nic, 1);
            ...
            handler.postDelayed(new Runnable() {
                public void run() {
                    ...
                    ONLINE_USERS = database.getReference(secret_field + "/Online_Users/" + my_nic);
                    ONLINE_USERS.child(my_nic).setValue("online");
                }
            }, 1000);
                                ...
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    ...
                                    key_public_Ref.addValueEventListener(new ValueEventListener() {
                                        ...
                                        public void onDataChange(@NotNull DataSnapshot dataSnapshot11) {
                                            ...
                                            } else { //Если публичного ключа собеседника в базе данных нет. Это свидетельствует о том, что он нас в список контактов еще не добавил.
    ...
    //Метод, создающий канал уведомлений.
    //Работает с android version > 8 (Oreo).
    private void createNotificationChannel(String interlocutor_name, String
            data_snapshotValue) {
    ...
    public void onStop() {
        ...
        try {
            ...
            //Код определяет, какая активность находится сейчас в стеке первой (перед пользователем).
            //ActivityManager am = (ActivityManager) this.getSystemService(ACTIVITY_SERVICE);
            //List<ActivityManager.RunningTaskInfo> taskInfo = am.getRunningTasks(1);
            //ComponentName componentInfo = taskInfo.get(0).topActivity;

            //Если активность - BottomNavigationActivity
            //if (!componentInfo.getClassName().equals("com.example.fludrex.BottomNavigationActivity")) {

            //Ставим статус - пользователь "онлайн" и отключаем activity
            ONLINE_USERS = database.getReference(secret_field + "/Online_Users/" + my_nic);
            ONLINE_USERS.child(my_nic).setValue(ServerValue.TIMESTAMP);
